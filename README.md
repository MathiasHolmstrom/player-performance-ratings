...
### Example: Comparing point estimates vs distributions

A key advantage of distribution estimators is flexibility: you can easily swap between a simple point estimator and a full distribution estimator to compare performance.
...
# Approach 1: Simple LGBMRegressor (point estimates only)
...
# (setup cross_validator and scorer as shown in previous examples)

# Results on NBA player points prediction:
# LGBMRegressor MAE:              4.5305
# NegativeBinomial Point Est MAE: 4.5305
# NegativeBinomial Ordinal Loss:  0.3786
```

**Key points:**
- `NegativeBinomialEstimator` requires a `point_estimate_pred_column` generated by a predictor_transformer
- Both approaches achieve the same MAE because they use the same base model
- Distribution approach provides additional capabilities:
  - Full probability distribution: `probabilities.sum(axis=1) == 1.0`
  - Uncertainty quantification via ordinal loss
  - Expected value calculation: `(probabilities * possible_values).sum(axis=1)`
  - Player-specific variance modeling with `r_specific_granularity`

See [examples/nba/cross_validation_example.py](examples/nba/cross_validation_example.py) for a complete runnable example with cross-validation and both approaches.
