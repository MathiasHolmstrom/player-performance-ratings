...
possessions = possessions.merge(lineups.rename(columns={'lineup': 'lineup_offense'}), left_on=['lineup_id_offense'] ,right_on=['lineup_id'], how='left')
possessions = possessions.merge(lineups.rename(columns={'lineup': 'lineup_defense'}), left_on=['lineup_id_defense'] ,right_on=['lineup_id'], how='left')

exploded = possessions.explode(['lineup_offense']).rename(columns={'lineup_offense': 'player_id'})
...
df = df[["team_id", "start_date", "game_id", "player_id", 'player_name', 'start_position', "team_id_opponent", "points",
         "game_minutes", 'pace', 'possessions',
         "minutes", "won", "plus_minus", "location", "score", "score_opponent", "team_possessions", "team_pace"]]
