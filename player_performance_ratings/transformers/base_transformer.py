...
class BaseLagGeneratorPolars():
    ...
    def _concat_df(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        return concat_df.unique(
            subset=[self.column_names.match_id, self.column_names.team_id, self.column_names.player_id]
            # maintain_order=True
        )
    ...
    def _store_df(self, df: pl.DataFrame, additional_cols_to_use: Optional[list[str]] = None):
        ...
        self._df = (self._df
                    .sort([self.column_names.match_id,
                           self.column_names.team_id,
                           self.column_names.player_id],
                          #   descending=True
                          )
                    .unique(subset=[self.column_names.match_id,
                                    self.column_names.team_id,
                                    self.column_names.player_id],
                            #  maintain_order=True
                            )
    ...
    def _create_transformed_df(self, df: pl.DataFrame, concat_df: pl.DataFrame) -> pl.DataFrame:
        ...
        transformed_df = concat_df.join(df.select(ori_cols), on=[cn.match_id, cn.player_id, cn.team_id], how='inner')
    ...
    def _add_opponent_features(self, df: pl.DataFrame) -> pl.DataFrame:
        team_features = df.groupby([self.column_names.team_id, self.column_names.match_id]).agg(
            **{col: pl.mean(col) for col in self._entity_features}
        )
        ...
        new_df = new_df.filter(pl.col(self.column_names.team_id) != pl.col('__opponent_team_id')).drop(['__opponent_team_id'])
    ...
    def _generate_future_feats(self, transformed_df: pl.DataFrame, ori_df: pl.DataFrame,
                               known_future_features: Optional[list[str]] = None) -> pl.DataFrame:
        known_future_features = known_future_features or []
        ori_cols = ori_df.columns
        cn = self.column_names

        transformed_df[self._entity_features] = transformed_df[self._entity_features].fill_nan(-999.21345)
        first_grp = transformed_df.group_by(self.granularity).agg([pl.col(f).first().alias(f) for f in self._entity_features])
        transformed_df = transformed_df[
            [c for c in transformed_df.columns if c not in self._entity_features]].join(first_grp,
                                                                                        on=self.granularity,
                                                                                        how='left')
        ...
        team_features = (
            transformed_df.group_by([self.column_names.team_id, self.column_names.match_id])
        ...
        opponent_feat_names = [f"{f}_opponent" for f in self._entity_features]
        new_df = transformed_df.join(df_opponent_feature, on=[self.column_names.match_id],
                                     suffix='_team_sum')
        new_df = new_df.filter(new_df[self.column_names.team_id] != new_df['__opponent_team_id'])
        ...
        first_grp = new_df.group_by('__opponent_team_id').agg([pl.col(f).first().alias(f) for f in opponent_feat_names])
        new_df = new_df[
            [c for c in new_df.columns if c not in opponent_feat_names]].join(first_grp,
        ...
        transformed_df = transformed_df.join(
            new_df.select([self.column_names.match_id, self.column_names.team_id, self.column_names.player_id,
                    *opponent_feat_names]),
            on=[self.column_names.match_id, self.column_names.team_id, self.column_names.player_id], how='left')

        ori_df = ori_df.with_columns(
            [pl.col(col).cast(pl.Utf8).alias(col) for col in [cn.match_id, cn.player_id, cn.team_id]]
        )
        ...
        transformed_df = ori_df[ori_feats_to_use].join(transformed_df[transformed_feats_to_use],
                                                       on=[cn.match_id, cn.team_id,
                                                           cn.player_id])

        return transformed_df.select(
            list(set(ori_cols + [f for f in self.features_out if f not in known_future_features])))
