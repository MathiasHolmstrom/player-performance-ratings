...
def test_lgbm_wrapper_requires_date_column():
    """Test that LGBMWrapper requires date_column in X during fit"""
    wrapper = LGBMWrapper(
...
def test_lgbm_wrapper_predict():
    ...
    X_pred = pd.DataFrame({
        "feature1": [4, 5],
        "date_col": pd.date_range('2024-01-04', periods=2, freq='D')
    })
...
def test_lgbm_wrapper_predict_proba():
    ...
    X_pred = pd.DataFrame({
        "feature1": [5, 6],
        "date_col": pd.date_range('2024-01-05', periods=2, freq='D')
    })
...
def test_granularity_predictor_predict_proba():
    ...
    X_train = pd.DataFrame({
        "position": ["a", "a", "b", "b", "a", "b"],
        "feature1": [0.1, 0.2, 0.5, 0.6, 0.15, 0.55]
    })
...
# ============================================================================
# GranularityEstimator Additional Tests
# ============================================================================

def test_granularity_estimator_predict_proba_raises_error():
    """Test GranularityEstimator predict_proba raises error if estimator doesn't support it"""
    predictor = GranularityEstimator(
        estimator=LinearRegression(),  # LinearRegression doesn't have predict_proba
        granularity_column_name="position"
    )
    ...
    with pytest.raises(AttributeError, match="does not have predict_proba"):
        predictor.predict_proba(X_pred)
...
def test_sklearn_wrapper_initialization():
    ...
    wrapper = SkLearnWrapper(estimator=LogisticRegression())
    assert wrapper.estimator is not None
