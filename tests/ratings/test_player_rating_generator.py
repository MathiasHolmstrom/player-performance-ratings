...
from spforge.ratings import PlayerRatingGenerator, RatingUnknownFeatures
...
def sequential_df(base_cn):
    ...
    return pl.DataFrame({
        ...
        "dt": ["2024-01-01", "2024-01-01", "2024-01-02", "2024-01-02", "2024-01-03", "2024-01-03"],
...
def test_fit_transform_updates_internal_state(base_cn, sample_df):
    ...
    gen = PlayerRatingGenerator(performance_column="perf", column_names=base_cn, auto_scale_performance=True)
...
def _create_date_column_player(date_format: str, dates: list) -> pl.Series:
    ...
    if date_format == "string_iso_date":
        ...
        date_strings = [f"{d} 12:00:00" if isinstance(d, str) and " " not in d else d for d in dates]
        return pl.Series("dt", date_strings)
    elif date_format == "date_type":
        from datetime import date
        date_objs = [date(2024, 1, int(d.split("-")[2])) if isinstance(d, str) and "-" in d else d for d in dates]
        return pl.Series("dt", date_objs, dtype=pl.Date)
    elif date_format == "datetime_type":
        from datetime import datetime
        datetime_objs = [datetime(2024, 1, int(d.split("-")[2])) if isinstance(d, str) and "-" in d else d for d in dates]
...
def test_fit_transform_when_date_formats_vary_then_processes_successfully_player(base_cn, date_format):
    """
...
def test_future_transform_when_date_formats_vary_then_processes_successfully_player(base_cn, date_format):
    """
    When date formats vary for player rating generator, then we should expect to see
...
def test_init_custom_suffix_applied_to_all_features(base_cn):
    ...
    gen = PlayerRatingGenerator(
        ...
        column_names=base_cn,
        output_suffix="v2",
        features_out=[RatingKnownFeatures.PLAYER_OFF_RATING, RatingKnownFeatures.TEAM_DEF_RATING_PROJECTED]
    )
...
def test_transform_error_before_fit(base_cn, sample_df):
    ...
    gen = PlayerRatingGenerator(
        performance_column="perf", 
        column_names=base_cn,
        features_out=[RatingKnownFeatures.PLAYER_OFF_RATING]  # Explicitly request player_off_rating
    )
...
def test_future_transform_extreme_rating_differences(base_cn):
    ...
    gen = PlayerRatingGenerator(
        performance_column="perf", 
        column_names=base_cn,
        non_predictor_features_out=[RatingUnknownFeatures.PLAYER_PREDICTED_OFF_PERFORMANCE]  # Request prediction column
    )
    ...
    gen._player_off_ratings["NOOB"] = PlayerRating(id="NOOB", rating_value=-100.0, confidence_sum=30)
    gen._player_def_ratings["NOOB"] = PlayerRating(id="NOOB", rating_value=-100.0, confidence_sum=30)
...
def test_fit_transform_multiple_calls_persistence(base_cn):
    ...
    df1 = pl.DataFrame({
        "pid": ["P1", "P2"], "tid": ["T1", "T2"], "mid": ["M1", "M1"], "dt": ["2024-01-01", "2024-01-01"],
        "perf": [0.8, 0.2], "pw": [1.0, 1.0]
    })
    df2 = pl.DataFrame({
        "pid": ["P1", "P2"], "tid": ["T1", "T2"], "mid": ["M2", "M2"], "dt": ["2024-01-02", "2024-01-02"],
        "perf": [0.8, 0.2], "pw": [1.0, 1.0]
    })
...
def test_fit_transform_auto_scale_false(base_cn):
    ...
    df = pl.DataFrame({
        "pid": ["P1", "P2"], "tid": ["T1", "T2"], "mid": ["M1", "M1"], "dt": ["2024-01-01", "2024-01-01"],
        "perf": [0.9, 0.1], "pw": [1.0, 1.0]
    })
    gen_no_scale = PlayerRatingGenerator(performance_column="perf", column_names=base_cn, auto_scale_performance=False)
...
def test_fit_transform_multiple_league_changes(base_cn):
    ...
    assert gen._player_off_ratings["P3"].rating_value < 1000.0  # Below default (started at 900, performed poorly)
...
@pytest.mark.parametrize("features_out,non_predictor_features_out,output_suffix,expected_cols", [
    ...
    (
        ...
        [RatingUnknownFeatures.PLAYER_PREDICTED_OFF_PERFORMANCE],
        "custom",
        ["player_off_rating_custom", "team_off_rating_projected_custom", "player_predicted_off_performance_custom"]
    ),
    ...
    (
        ...
        None,
        ["team_off_rating_projected_perf", "opponent_def_rating_projected_perf", "team_rating_perf"]
    ),
    ...
    (
        ...
        [RatingUnknownFeatures.PLAYER_PREDICTED_OFF_PERFORMANCE],
        None,
        ["rating_difference_projected_perf", "player_predicted_off_performance_perf"]  # Default is RATING_DIFFERENCE_PROJECTED
    ),
])
