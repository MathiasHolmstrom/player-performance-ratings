...
from spforge.transformers import RollingMeanTransformer, LagTransformer
...
def test_nba_integration_pipeline_cross_validate_train_future_predict():
    ...
    min_validation_date = data.filter(pl.col(column_names.match_id)==hist_game_ids[len(hist_game_ids)-20])[column_names.start_date].min()
    ...
    validation_predictions = validation_future_data.select(
        [pipeline.pred_column, column_names.match_id, column_names.player_id]).rename({pipeline.pred_column: 'cv_prediction'})

    future_data_with_cv_predictions = future_data.join(validation_predictions, on=[column_names.match_id, column_names.player_id])
