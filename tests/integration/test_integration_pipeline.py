...
from spforge.transformers import RollingMeanTransformer, LagTransformer, RollingMeanDaysTransformer
...
def test_nba_integration_pipeline_cross_validate_train_future_predict():
    ...
    min_validation_date = data.filter(pl.col(column_names.match_id) == hist_game_ids[len(hist_game_ids) - 20])[
    ...
    validation_future_data = validation_data.filter(pl.col(column_names.match_id).is_in(game_ids[-20:]))
    future_data = future_data.with_columns(pl.col(pipeline.features).fill_nan(-999992))
    validation_future_data = validation_future_data.with_columns(pl.col(pipeline.features).fill_nan(-999992))
    validation_cols_renamed = {c: f"{c}_cv" for c in pipeline.features}
    validation_future_player_grouped = (validation_future_data
                                        .group_by(column_names.player_id)
                                        .agg(pl.col(pipeline.features)
                                             .first())
                                        .rename(validation_cols_renamed)
                                        )
    future_joined_data = validation_future_player_grouped.join(future_data, on=column_names.player_id)


    for feat in pipeline.features:
        assert future_joined_data[f"{feat}_cv"].to_list() == future_joined_data[feat].to_list()

    validation_predictions = validation_future_data.select(
        [pipeline.pred_column, column_names.match_id, column_names.player_id]).rename(
        {pipeline.pred_column: 'cv_prediction'})

    future_data_with_cv_predictions = future_data.join(validation_predictions,
    ...
    mean_future_prediction = future_data_with_cv_predictions[pipeline.pred_column].mean()
...
def test_lol_integration_pipeline_cross_validate_train_future_predic():
    ...
    data = data.assign(team_count=data.groupby("gameid")["teamname"].transform("nunique")).loc[
    ...
    pipeline = Pipeline(
        ...
        lag_transformers=[
            ...
            LagTransformer(features=["kills"], lag_length=3, granularity=[column_names.player_id]),
        ],
        predictor=SklearnPredictor(
            estimator=LGBMRegressor(max_depth=2, random_state=42, verbose=-100),
            target="kills",
    ...
    abs_diff_mean = future_data_with_cv_predictions['abs_diff'].mean()
    assert abs_diff_mean < 0.25
    mean_cv_prediction = future_data_with_cv_predictions['cv_prediction'].mean()
    mean_future_prediction = future_data_with_cv_predictions[pipeline.pred_column].mean()
    assert abs(mean_cv_prediction - mean_future_prediction) < 0.05
