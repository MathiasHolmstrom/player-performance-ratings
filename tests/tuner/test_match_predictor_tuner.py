...
def test_match_predictor_tuner():
    ...
    performance_search_range = PerformancesSearchRange(
        search_ranges=[
            ParameterSearchRange(name="won", type="uniform", low=0.8, high=1)
        ]
    )

    performances_generator_tuner = PerformancesGeneratorTuner(
        performances_search_range=performance_search_range, n_trials=1
    )
    ...
    assert (
        deep_equals(best_rating_model.rating_generators, pipeline.rating_generators)
        == False
    )
    assert (
        deep_equals(
            best_rating_model.performances_generator, pipeline.performances_generator
        )
        == False
    )
