...
from player_performance_ratings.predictor_transformer._simple_transformer import AggregatorTransformer, \
...
def test_aggregator_transformer_granularity_sum(df_type):
    ...
    transformer = AggregatorTransformer(columns=['minutes'], granularity=['game_id', 'team_id'],
                                        column_to_alias={'minutes': 'minutes_sum'}, aggregator='sum')
...
def test_aggregator_transformer_sum(df_type):
    ...
    transformer = AggregatorTransformer(columns=['minutes'], column_to_alias={'minutes': 'minutes_sum'},
                                        aggregator='sum')
    transformed_df = transformer.transform(df)
...
def test_aggregator_transformer_mean(df_type):
    ...
    transformer = AggregatorTransformer(columns=['minutes', 'minutes_sum'],
...
def test_normalizer_to_column_transformer(df_type):
    ...
    expected_df['predicted_minutes'] = [10 * game_id_1_team_1_ratio, 20 * game_id_1_team_1_ratio,
                                        30 * game_id_1_team_2_ratio, 40 * game_id_1_team_2_ratio,
                                        40 * game_id_2_team_1_ratio, 30 * game_id_2_team_1_ratio,
                                        30 * game_id_2_team_2_ratio, 40 * game_id_2_team_2_ratio]
