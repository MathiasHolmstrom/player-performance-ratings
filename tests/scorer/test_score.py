...
SCORER_VALIDATION_CASES = [
    pytest.param(
        lambda: MeanBiasScorer(pred_column="pred", target="target"),
        lambda: pd.DataFrame(
            {
                "pred": [2.0, 0.0],
                "target": [1.0, 2.0],
    ...
    pytest.param(
        lambda: PWMSE(pred_column="pred", target="target", labels=[0, 1]),
        lambda: pd.DataFrame(
            {
    ...
    pytest.param(
        lambda: SklearnScorer(
            scorer_function=mean_absolute_error, pred_column="pred", target="target"
        ),
        lambda: pd.DataFrame(
            {
                "pred": [1.0, 0.0],
    ...
    pytest.param(
        lambda: ProbabilisticMeanBias(
            pred_column="pred", target="target", class_column_name="classes"
        ),
        lambda: pd.DataFrame(
            {
    ...
    pytest.param(
        lambda: OrdinalLossScorer(pred_column="pred", target="target", classes=[0, 1]),
        lambda: pd.DataFrame(
            {
                "pred": [[0.2, 0.8], [0.6, 0.4]],
                "target": [1, 0],
...
def test_scorers_filter_is_validation_by_default(scorer_factory, df_factory):
    """Scorers should filter on `is_validation == 1` when the column is present."""
    df = df_factory()
