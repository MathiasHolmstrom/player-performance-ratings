...
import polars as pl
import pytest
from sklearn.metrics import log_loss, mean_absolute_error, mean_squared_error, root_mean_squared_error
...
def test_sklearn_scorer_with_params(df_type):
    ...
    scorer = SklearnScorer(
        pred_column="pred",
        scorer_function=root_mean_squared_error,
        target="target"
    )
    score = scorer.score(df)
...
@pytest.mark.skip(reason="ProbabilisticMeanBias.score() calls apply_filters which uses narwhals, but score() expects pandas DataFrame - implementation bug")
@pytest.mark.parametrize("df_type", [pd.DataFrame])
