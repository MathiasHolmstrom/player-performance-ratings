...
def test_ordinal_loss_scorer_multiclass_rename_class_column_name():
    ...
    score = OrdinalLossScorer(
        pred_column="predictions", class_column_name="total_points_classes"
    ).score(data)
    assert score > 0
    assert score < 0.693
...
def test_ordinal_loss_scorer_multiclass():
    ...
    score = OrdinalLossScorer(pred_column="predictions").score(data)
    assert score > 0
    assert score < 0.693
...
def test_sk_learn_scorer_mean_absolute_error(df):
    score = SklearnScorer(
        pred_column="predictions", scorer_function=mean_absolute_error
    ).score(df)
...
def test_mean_absolute_error(df):
    ...
    scorer = MeanBiasScorer(pred_column="predictions")
