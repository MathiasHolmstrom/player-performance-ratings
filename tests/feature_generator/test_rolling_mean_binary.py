...
def test_rolling_mean_binary__stores_only_last_window_rows_per_entity(column_names: ColumnNames):
    historical_df = pd.DataFrame(
        {
            "player": ["a", "b"] * 3,
            "game": ["1", "1", "2", "2", "3", "3"],
            "score_difference": [1, 10, 2, 20, 3, 30],
            "won": [1, 0, 1, 0, 1, 0],
...
def test_rolling_mean_binary__future_transform_uses_trimmed_state(column_names: ColumnNames):
    historical_df = pd.DataFrame(
        {
            "player": ["a", "b", "a", "b", "a", "b"],
            "game": ["1", "1", "2", "2", "3", "3"],
    ...
    expected_df = future_df.assign(
        **{
            f"{transformer.prefix}_score_difference2_1": [2.5, None],
            f"{transformer.prefix}_score_difference2_0": [None, 25.0],
        }
    )
    pd.testing.assert_frame_equal(
