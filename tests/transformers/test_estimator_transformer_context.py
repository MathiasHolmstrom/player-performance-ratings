...
from spforge.estimator import SkLearnEnhancerEstimator, GranularityEstimator
...
def test_estimator_transformer_context__nested_estimators():
    ...
    inner = SkLearnEnhancerEstimator(
        estimator=LinearRegression(),
        date_column="game_date"
    )
    grouped = GranularityEstimator(
        estimator=inner,
        granularity_column_name="team_id"
    )
    transformer = EstimatorTransformer(
        estimator=grouped,
        prediction_column_name="pred",
