...
class SklearnPredictor(BasePredictor):
    ...
    def train(self, df: FrameT, features: Optional[list[str]] = None) -> None:
        ...
        if self.granularity:
            ...
            assert len(filtered_df) < row_count_before_grouping, (
                f"Row count after grouping is {len(filtered_df)} which is greater than the row count before grouping {row_count_before_grouping}."
    ...
    def predict(
        ...
        if self.granularity:
            ...
            assert len(grouped_df) < row_count_before_grouping, (
                f"Row count after grouping is {len(grouped_df)} which is greater than the row count before grouping {row_count_before_grouping}."
                f" This is likely a consequence of the categorical {self._cat_feats} features not matching the granularity {self.granularity} "
            )
        else:
            grouped_df = df
