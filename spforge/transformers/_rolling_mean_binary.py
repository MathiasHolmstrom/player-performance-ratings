...
class BinaryOutcomeRollingMeanTransformer(BaseLagGenerator):
    ...
    def transform_historical(self, df: FrameT, column_names: Optional[ColumnNames] = None) -> IntoFrameT:
        ...
        if self.column_names:
            ...
            transformed_df = self._create_transformed_df(df=df, concat_df=concat_df, match_id_join_on=self.column_names.update_match_id)
        else:
            transformed_df = self._concat_with_stored_and_calculate_feats(df).sort('__row_index')
            transformed_df = df.join(transformed_df.select(['__row_index', *self.features_out]), on='__row_index',
                                     how='left')
    ...
    def transform_future(self, df: FrameT) -> IntoFrameT:
        ...
        transformed_future = transformed_future.select([*input_cols, *self.features_out])
